...
import apw.codebase.gallery.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity(), ColorPickerDialogListener{
    ...
    private fun requestPermissions() {
        ActivityCompat.requestPermissions(this,
            arrayOf(Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.READ_MEDIA_IMAGES, Manifest.permission.READ_MEDIA_VIDEO),
            PERMISSION_REQUEST_PERM
        )
    }
    ...
    private fun loadImages() {
        binding.bottomNavigation.setOnItemSelectedListener { item ->
            when (item.itemId) {
                R.id.images -> {
                    binding.collapse.title = "Gallery"
                    setCurrentFragment(MainFrag())
                    true
                }
                R.id.albums -> {
                    binding.collapse.title = "Albums"
                    setCurrentFragment(Album())
                    true
                }
                R.id.search ->{
                    binding.collapse.title = "Search"
                    setCurrentFragment(Search())
                    true
                }
                R.id.utilities ->{
                    binding.collapse.title = "Utils"
    ...
    override fun onColorSelected(dialogId: Int, color: Int) {
        ...
        if (dialogId == 0) {
            ...
            GenerateQuoteImage().createQuoteImage(this, colorHex, "RobotoSlab.ttf")
        }
    }
